...
$result = mysqli_query($conn, "SELECT * FROM pemesanan as a JOIN pelanggan as b ON (a.id_pelanggan = b.id_pelanggan) WHERE id_pemesanan='$id'");
...
if(mysqli_num_rows($result) == 0){
    ...
    header ('location:pemesanan_form.php');
   }else{
...
if(isset($_POST["kirim"])){
    ...
    $koneksi = mysqli_query($conn, "INSERT INTO pembayaran (id_pemesanan,id_pegawai, tgl_pembayaran, total_pembayaran, 
                            status_pembayaran, nama_bank, atas_nama) VALUES ('$id_pemesanan','$id_pegawai','$tgl_pembayaran','$total_pembayaran',
                            '$status_pembayaran','$nama_bank','$atas_nama') ");

 
//update status transaksi jadi detail, update status pembayaran jadi kirim dan nota
    $koneksi = mysqli_query($conn, "UPDATE pemesanan SET status_transaksi=0 WHERE id_pemesanan='$id_pemesanan' ");
    //jika query update sukses
    if($koneksi){
  
    echo 'Data berhasil di simpan! ';  //Pesan jika proses simpan sukses
    header("location:pembayaran.php"); //membuat Link untuk kembali ke halaman edit
                    ...
                    <div class="page-title">
                        <div class="row">
                            ...
                            <div class="col-12 col-md-6 order-md-1 order-last">
                                <h3>Form Input Pembayaran - SHEEPSELL</h3>
                                <p class="text-subtitle text-muted">bayar domba kamu disini</p>
                            </div>
                    ...
                    <section class="section">
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Pembayaran Domba offline</h4>
                            </div>
                           
                            <div class="card-body col-md-4 offset-md-4 py-4">
                           
                            <div class="class alert-info">
                            Tanggal          : <?=$data['tgl_pesan']?><br>
                            Nama Pelanggan   : <?=$data['nama_pelanggan']?><br>
                            </div>
                            <div class="class alert-success">
                            Jenis Bayar      : <?=$data['jenis_bayar']?><br>
                            Total Tagihan Rp. <?=number_format($data['total_harga']+$data['ongkos_kirim'])?>
                            </div>

                            <form action="pembayaran_form.php" method="post">

                                <div class="form group">
                                    <label>ID Pemesanan</label>
                                    <input type="text" class="form-control" name="id_pemesanan" value="<?=$data['id_pemesanan']?>" readonly>
                                </div>
                                ...
                                <div>
                                        <input type="submit" name="kirim" class="btn btn-warning rounded-pill"></a>
                                        </div> 
                            </form>
                            </div>
                        </div>
